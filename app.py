...
# --- INICIO BLOQUE AÑADIDO: OCR Y DETECCIÓN DE IDIOMA (agregado en esta sesión) ---
def _detect_language(text: str) -> str:
    """Detecta idioma dominante del texto usando langdetect (si está presente)."""
    try:
...
def _read_pdf_text_robust(abs_path: str, force_ocr: bool = False) -> Tuple[str, str]:
    ...
    try:
        ...
        if proc.returncode == 0 and os.path.exists(out_pdf):
            ...
            if "qpdf" in stderr.lower():
                diag.append("ocrmypdf: parece faltar 'qpdf' (instálalo con choco install qpdf)")
            if "ghostscript" in stderr.lower() or "gswin" in stderr.lower():
                diag.append("ocrmypdf: parece faltar 'ghostscript' (instálalo con choco install ghostscript)")
    except Exception as e:
    ...
    return "", "\n".join(diag)
# --- FIN BLOQUE AÑADIDO: OCR Y DETECCIÓN DE IDIOMA ---
...
def render_pantalla_8_ia():
    ...
    st.caption("Pantalla 8 · Selecciona EETT de Biblioteca · Lee WORD (.docx) o PDF (incluye OCR en español) y genera resumen + checklist QA/QC (sin inventar).")
